...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt($ch, CURLOPT_USERPWD, $sk . ':' . '');
curl_setopt($ch, CURLOPT_POSTFIELDS,'amount=100&currency=usd&payment_method_types[]=card&description=&payment_method='.$tok1.'&confirm=true&off_session=true');
$r2 = curl_exec($ch);
...
if (strpos($r2, '"seller_message": "Payment complete."')){
 ...
 // hits_sender($tg_user_id, "𝙇𝙞𝙫𝙚",$lista, $resmsg, $risklevel);
  echo '<p style="text-align: left;" class="uk-margin-small-top"><b>┃'.$status.' '. $lista . ' <br>┃ '.$resmsg.': <a class="receipt" href="'.$receipturl.'">𝙂𝙚𝙩 𝙍𝙚𝙘𝙚𝙞𝙥𝙩</a> <br>┃ 𝙍𝙞𝙨𝙠 𝙇𝙚𝙫𝙚𝙡 : '.$risklevel.' </b></p>';
...
function decline_reason($a,$b) {
 return $a." ".$b." ";
}
function decline_msg($a,$b,$c,$d) {
